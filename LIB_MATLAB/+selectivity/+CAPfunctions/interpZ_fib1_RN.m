function Vq = interpZ_fib1_RN (CoordonneesXY,CoordonneesZ,current_density_T,x,y,z)
%--------------------------------------------------
...
init_1stNR = 0;
% Z: 1x nNodes
V = current_density_T{1}; % Density only in the nth fiber

% NRwidth = 20e-6;
NRlength = 20e-6;
internodeL = 100*3e-6;

Vq = interp1(Z, V, z, 'nearest');

Vq(mod(round(z*1e6), internodeL*1e6)>NRlength) = 0;


Vq(isnan(Vq))=0;
% Vq(abs(x)>NRwidth*1.5)=0;
% Vq(abs(y)>NRwidth*1.5)=0; % density only in one fiber width of y
end