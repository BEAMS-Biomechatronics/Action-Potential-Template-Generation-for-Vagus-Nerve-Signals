...
% modif_impedance.m
%
% Model exported on May 5 2017, 13:56 by COMSOL 5.2.0.166.
...
model.modelPath('D:\Nicolas\Dropbox\Dropbox\0_THESE\CODE\LIB_MATLAB');
...
model.param.set('nerveD', '0.00135[m]');
...
model.param.set('ringD', '0.002000[m]');
model.param.set('nerveL', '0.040000[m]');
model.param.set('ringTorusD', '0.000030[m]');
model.param.set('ringCuffD', '0.0028500000[m]');
model.param.set('courant', '1.00000[A]');
model.param.set('cuffL', '0.005100[m]');
model.param.set('LayerBC', '100[mm]');
model.param.set('alpha', '-86.350000[m]');
model.param.set('sigmaConjonctive', '0.160000[m]');
...
model.geom('geom1').feature('wp1').set('quickz', 'nerveL/2');
...
model.geom('geom1').feature('pc1').set('parmax', 'nerveL/2');
model.geom('geom1').create('ps1', 'ParametricSurface');
model.geom('geom1').feature('ps1').set('selresult', 'on');
model.geom('geom1').feature('ps1').set('coord', {'-alpha/1[m]*(s2)^2+s1' '0' 's2'});
...
model.physics('ec').feature('ein2').selection.set([5 6 193 216]);
...
model.physics('ec').feature('ci1').set('ds', ['50[' native2unicode(hex2dec({'00' 'b5'}), 'unicode') 'm]']);
